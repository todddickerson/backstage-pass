# Ejected from `bullet_train-1.29.0/app/controllers/concerns/account/teams/controller_base.rb`.
...
module Account::Teams::ControllerBase
  ...
  def show
    ...
    # Load spaces for the team dashboard
    @spaces = @team.spaces

    # Load access grants for buyers
    @access_grants = current_user.access_grants.active.includes(:access_pass)

    # Load purchases
    @billing_purchases = Billing::Purchase.where(user: current_user) if defined?(Billing::Purchase)
  end
